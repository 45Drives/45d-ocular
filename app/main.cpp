...
LONG WINAPI UnhandledExceptionHandler(struct _EXCEPTION_POINTERS *ExceptionInfo)
{
    ...
    swprintf_s(dmpFileName, L"%ls\\Ocular-%I64u.dmp",
...
int main(int argc, char *argv[])
{
    ...
    QCoreApplication::setOrganizationName("Ocular Project");
    QCoreApplication::setOrganizationDomain("45drives.com");
    QCoreApplication::setApplicationName("Ocular");
...
#endif
    {
        s_LoggerFile = new QFile(tempDir.filePath(QString("Ocular-%1.log").arg(QDateTime::currentSecsSinceEpoch())));
    ...
    QStringList existingLogNames = tempDir.entryList(QStringList("Ocular-*.log"), QDir::NoFilter, QDir::SortFlag::Time);
    ...
    if (WMUtils::isRunningWindowManager()) {
        ...
        if (!qEnvironmentVariableIsSet("QT_QPA_PLATFORM")) {
            ...
            if (!QFile("/dev/dri").exists()) {
                ...
                qWarning() << "On the Raspberry Pi, you must enable the 'fake KMS' driver in raspi-config to use Ocular outside of the GUI environment.";
            }
    ...
    if (!qEnvironmentVariableIsSet("QT_OPENGL")) {
        ...
        // seem to crash Ocular far more often than DirectX.
        qputenv("QT_OPENGL", "angle");
    }
#endif

#if !defined(Q_OS_WIN32) || QT_VERSION >= QT_VERSION_CHECK(6, 0, 0)
    // Ocular requires the non-threaded renderer because we depend
    ...
    // We don't want this all the time that Ocular is open. We will set
    ...
    SDL_SetHint(SDL_HINT_AUDIO_DEVICE_APP_NAME, "Ocular");
    SDL_SetHint(SDL_HINT_APP_NAME, "Ocular");
    ...
    app.setDesktopFileName("com.moonlight_stream.Ocular");
    qputenv("SDL_VIDEO_WAYLAND_WMCLASS", "com.moonlight_stream.Ocular");
    qputenv("SDL_VIDEO_X11_WMCLASS", "com.moonlight_stream.Ocular");
